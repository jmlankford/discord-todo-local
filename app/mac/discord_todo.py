"""
Discord To-Do Desktop Widget — v3
- Checkboxes per task + "Tasks Completed" button
  → deletes checked tasks from Discord, posts ✅ completion message as taskbot
- "Completed?" header with select-all toggle
...
class DiscordTodoApp:
    def __init__(self):
        ...
        self._thread_id    = None   # cached after successful fetch

        # checkbox state
        self._check_vars   = {}     # {task_id: BooleanVar}
    ...
    def _wrap_width(self) -> int:
        if self._canvas:
            ...
            if w > 10:
                return max(80, w - 50)  # accounts for checkbox width
        return 210
    ...
    def fetch_messages(self):
        ...
        self._thread_id = thread_id   # cache for completions
        ...
        filtered = [
            ...
            if not m.get('content', '').strip().startswith('✅')
    ...
    def _delete_message(self, thread_id: str, msg_id: str) -> bool:
        try:
            r = requests.delete(
                f"{DISCORD_API}/channels/{thread_id}/messages/{msg_id}",
                headers=self._headers, timeout=10)
            return r.status_code == 204
        except requests.RequestException:
            return False

    def _post_completion(self, thread_id: str, task_content: str) -> bool:
        thread_name = self.config.get('thread_name', 'Josh')
        content = f"✅ **{thread_name}** completed: **{task_content}**"
        try:
            r = requests.post(
    ...
    def _post_task(self, content: str) -> bool:
    ...
    def run_widget(self):
        ...
        _tbtn('+', self._add_task_dialog            ).pack(side='right', pady=6)
        ...
        self._refresh_btn = tk.Button(
            self._bbar, text='↻', command=self._manual_refresh,
            font=('Segoe UI', 9), bg=bbar_bg, fg='#111111',
            relief='flat', bd=0, cursor='hand2', activebackground=darken(bbar_bg))
        self._refresh_btn.pack(side='right', padx=(0, 3), pady=4)

        self._complete_btn = tk.Button(
            self._bbar, text='✓ Tasks Completed',
            command=self._complete_tasks,
            font=('Segoe UI', 8, 'bold'), bg='#1a6b1a', fg='#ffffff',
            relief='raised', bd=2, cursor='hand2',
            activebackground='#145214', activeforeground='#ffffff')
        self._complete_btn.pack(side='right', padx=(0, 8), pady=3)
    ...
    def _draw_tasks(self):
        ...
        if not self.tasks:
            ...
            # ── "Completed?" header ───────────────────────────────────────────
            hdr = tk.Frame(self.task_frame, bg=bg)
            hdr.pack(fill='x', padx=6, pady=(4, 0))

            tk.Label(
                hdr, text="Completed?",
                font=('Segoe UI', 8, 'bold'), bg=bg, fg=fg,
            ).pack(side='right')

            # Thin separator
            tk.Frame(self.task_frame, bg=darken(bg, 0.82), height=1).pack(
                fill='x', padx=6, pady=(2, 4))

            # ── Task rows ─────────────────────────────────────────────────────
            for task in self.tasks:
                var = tk.BooleanVar(value=False)
                self._check_vars[task['id']] = var
                ...
                tk.Checkbutton(
                    ...
                    selectcolor=darken(bg, 0.88),
                    relief='flat', bd=0
                ).pack(side='right', anchor='n', pady=3)
    ...
    def _complete_tasks(self):
        checked = []
        for t in self.tasks:
            var = self._check_vars.get(t['id'])
            if var and var.get():
                checked.append(t)

        if not checked:
            if self.sync_var:
                self.sync_var.set("No tasks selected.")
            return

        if self.sync_var:
            self.sync_var.set(f"Completing {len(checked)} task(s)…")
        if self._complete_btn:
        ...
        def do_complete():
            ...
            if not thread_id:
                ...
                if err:
                    if self.root:
                        self.root.after(0, lambda e=err: self._on_complete_done(0, 0, e))
                    return
                self._thread_id = thread_id
            ...
            errors = 0
            for task in checked:
                deleted = self._delete_message(thread_id, task['id'])
                posted  = self._post_completion(thread_id, task['content'])
                if deleted and posted:
                    ok += 1
                else:
                    errors += 1

            if self.root:
                self.root.after(0, lambda o=ok, e=errors: self._on_complete_done(o, e))

        threading.Thread(target=do_complete, daemon=True).start()

    def _on_complete_done(self, ok: int, errors: int, err_msg: str = None):
        if self._complete_btn:
            self._complete_btn.config(state='normal')
        if err_msg:
            if self.sync_var:
                self.sync_var.set(f"Error: {err_msg[:50]}")
            return
        msg = f"✓ Completed {ok}"
        if errors:
            msg += f"  ({errors} error{'s' if errors != 1 else ''})"
        if self.sync_var:
            self.sync_var.set(msg)
    ...
    def _add_task_dialog(self):
        ...
        tk.Label(dlg, text="Add Task", font=('Segoe UI', 12, 'bold'),
                 bg='#12121e', fg='#FFD500').pack(pady=(16, 8))

        entry = tk.Text(dlg, font=('Segoe UI', 10), bg='#1e1e34', fg='#ffffff',
                        insertbackground='#FFD500', relief='flat', bd=6,
                        height=3, wrap='word')
        entry.pack(padx=20, fill='x', ipady=4)
        entry.focus_set()

        status_var = tk.StringVar()
        tk.Label(dlg, textvariable=status_var, font=('Segoe UI', 8),
                 bg='#12121e', fg='#ff6b6b').pack(pady=(4, 0))

        def submit():
            content = entry.get('1.0', 'end').strip()
            if not content:
                status_var.set("Please enter a task.")
                return
            btn.config(state='disabled', text='Posting…')
            dlg.update()

            def do_post():
                ok = self._post_task(content)
                def on_done():
                    if ok:
                        dlg.destroy()
                        threading.Thread(target=self._bg_refresh, daemon=True).start()
                    else:
                        btn.config(state='normal', text='Add Task')
                        status_var.set("Failed to post. Check connection.")
                dlg.after(0, on_done)
            threading.Thread(target=do_post, daemon=True).start()

        btn = tk.Button(dlg, text="Add Task", command=submit,
                        font=('Segoe UI', 10, 'bold'), bg='#FFD500', fg='#000000',
                        relief='flat', padx=14, pady=6, cursor='hand2',
                        activebackground='#e6c200')
        btn.pack(pady=8)
        dlg.bind('<Control-Return>', lambda e: submit())
